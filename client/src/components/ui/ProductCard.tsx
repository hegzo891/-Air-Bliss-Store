...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const isSoldOut = (product.quantityAvailable ?? 0) <= 0;

  return (
    <div className={`group bg-card rounded-2xl overflow-hidden shadow-md border border-border/50 hover:shadow-xl hover:border-primary/20 transition-all duration-500 flex flex-col h-full transform hover:-translate-y-1 ${isSoldOut ? 'opacity-80' : ''}`}>
      <Link href={`/product/${product.id}`} className="block relative aspect-[4/5] overflow-hidden bg-muted/30">
        <img
          src={product.imageUrl}
          alt={name}
          className={`object-cover w-full h-full group-hover:scale-110 transition-transform duration-700 ease-out ${isSoldOut ? 'grayscale-[30%]' : ''}`}
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
        {isSoldOut && (
          <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
            <span className="bg-red-600 text-white text-sm font-bold uppercase tracking-widest px-5 py-2.5 rounded-full shadow-xl backdrop-blur-md flex items-center gap-2">
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="flex items-center justify-between mt-auto pt-5 border-t border-border/40">
          <div className="flex flex-col">
            ...
            <span className={`font-bold text-lg ${isSoldOut ? 'text-muted-foreground line-through' : 'text-foreground'}`}>
              {price} <span className="text-xs font-medium opacity-70">{t.common.price}</span>
            </span>
          </div>

          {isSoldOut ? (
            <div className="flex items-center justify-center w-10 h-10 rounded-full bg-muted text-muted-foreground cursor-not-allowed" title="Sold Out">
